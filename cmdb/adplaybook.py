...
loader = DataLoader()
options = Options(connection='ssh',gather_facts='no',remote_user='root',ack_pass=None,sudo_user='root',forks=5, sudo='yes', ask_sudo_pass=False, verbosity=5,module_path=['/usr/local/lib/python2.7/site-packages/ansible/modules'],become=True, become_method='sudo', become_user='root',check=None, listhosts=False,listtasks=False, listtags=None, syntax=None,diff=False)
...
def tasksbook(file,hosts):
    inventory = InventoryManager(loader=loader, sources='/home/wangyue/devops/dev/'+hosts)
    variable_manager = VariableManager(loader=loader, inventory=inventory)
    playbook = PlaybookExecutor(
              playbooks=['/home/wangyue/devops/dev/'+file],
              inventory=inventory,
